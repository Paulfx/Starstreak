...
- Mode Jouer : 
	Vous devez sélectionner le mode « jouer » sur le premier menu , sélectionner une 	chanson et une difficulté. La partie se lance.
	...
	Vous ne pouvez jouer une chanson que si la partition existe (de base, on vous en 	donne) 
- Mode Créer :
	Vous devez sélectionner le mode « créer » sur le premier menu, sélectionner une 	chanson et une difficulté. Attention la création effacera les partitions de 		la même chanson et de la même difficulté. Le partie se lance. 
...
Chaque ligne suivante correspond au notes jouées (00000) aucune note, (11111) toutes les notes et à la durée écoulée depuis le début de la musique. 

L’index correspond à chaque musique disponible avec les emplacements : 
	- des fichiers sons
	- des images
...
Les fichiers sources se trouvent dans le répertoire "src" de l’archive, séparés en /Core et /sdl2. Les tests de regressions (mains) sont dans le dossier src
Le répertoire data contient toute les données nécessaires au bon fonctionnement du jeux. 
Dans cette archive vous n’aurez que 2 chansons jouables (+1 menu)
Une troisième est disponible sur le lien de téléchargement dans le répertoire data/wav. (.wav à insérer dans data/wav)
Vous trouverez dans data/songs/ les partitions disponibles (nom/nom+difficulté).
l’executable est dans le dossier bin. 
